    ...
    useEffect(() => {
        if (step === 2) {
            get(ref(db, "books")).then(async (snapshot) => {
                ...
                const filtered = Object.values(data).filter(book =>
                    book.subject === genre &&
                    (
                        (lengthCategory === "Short" && book.number_of_pages < 150) ||
                        (lengthCategory === "Medium" && book.number_of_pages >= 150 && book.number_of_pages <= 300) ||
                ...
                <div className="fixed bottom-20 right-6 bg-[rgba(var(--bookcard),1)] rounded-xl shadow-lg w-96 max-w-full p-6 z-50 transition-colors">
                    ...
                    {step === 0 && (
                        <div>
                            ...
                            <p className="mb-3 text-center">What genre are you in the mood for today?</p>
                            <div className="flex flex-col gap-2 px-2">{["Fantasy", "Mystery", "Romance", "Sci-Fi"].map((g) => (
                                <button key={g} onClick={() => handleGenreSelect(g)} className="min-w-[80px] min-h-[40px] gap-2 bg-[rgb(45,140,169)] hover:bg-[rgb(39,65,84)] text-white text-sm rounded-full ">
                                    {g}
                                </button>
                            ))}
                            </div>
                    ...
                    {step === 1 && (
                        <div>
                            ...
                            <h2 className="text-xl font-bold mb-4 text-center">Book Recommendation</h2>
                            <p className="mb-2 text-center">How long a book are you up for?</p>
                            <div className="flex flex-col gap-2 px-2">
                                {availableLengths.map((l) => (
                                    <button
                                        key={l}
                                        onClick={() => handleLengthSelect(l)}
                                        className="bg-[rgb(45,140,169)] hover:bg-[rgb(39,65,84)] text-white text-sm rounded"
                                    >
                                        {l}
                                    </button>
                                ))}
                            ...
                            {book ? (
                                <>
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-4 py-2 bg-[rgb(45,140,169)] hover:bg-[rgb(39,65,84)] text-white font-semibold rounded shadow"
                                    >
                                    ...
                                    <button
                                        onClick={resetBot}
                                        className="mt-2 px-4 py-2 bg-gray-300 hover:bg-gray-400 text-gray-800 font-medium rounded"
                                    >
                                        New Recommendation
                                    </button>
                                </>
                                ...
                                <>
                                    ...
                                    <p className="text-gray-600">Try selecting a different length</p>
                                </>
                            )}
                        </div>
                    )}
